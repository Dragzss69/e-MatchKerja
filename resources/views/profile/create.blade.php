...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-indigo-600 to-indigo-700 px-6 py-5">
            <h1 class="text-xl font-bold text-white">📝 Lengkapi Profil Anda</h1>
            <p class="text-indigo-100 text-sm mt-1">Data diri Anda diperlukan untuk pengajuan bantuan dan rekomendasi pekerjaan</p>
        </div>

        <!-- Form -->
        <form action="{{ route('jobseeker.profile.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="status_kerja_saat_ini" class="w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm" required>
                            ...
                            <option value="bekerja_paruh_waktu" {{ old('status_kerja_saat_ini', $profile->status_kerja_saat_ini ?? '') == 'bekerja_paruh_waktu' ? 'selected' : '' }}>Bekerja Paruh Waktu</option>
                            <option value="bekerja_penuh" {{ old('status_kerja_saat_ini', $profile->status_kerja_saat_ini ?? '') == 'bekerja_penuh' ? 'selected' : '' }}>Bekerja Penuh</option>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="jumlah_tanggungan" value="{{ old('jumlah_tanggungan', $profile->jumlah_tanggungan ?? '0') }}" 
                               class="w-full rounded-xl border-slate-200 focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                               min="0" required>
                ...
                <div>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_penerima_bansos_lain" value="1" 
                               {{ old('is_penerima_bansos_lain', $profile->is_penerima_bansos_lain ?? false) ? 'checked' : '' }}
            ...
            <div class="pt-4 border-t">
                <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl transition flex items-center justify-center gap-2">
