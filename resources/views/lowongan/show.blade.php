            ...
            <div class="rounded-3xl bg-white border border-slate-200/80 shadow-sm overflow-hidden p-8 space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <p class="text-sm font-semibold text-indigo-600 flex items-center gap-1.5">
                        <i class="fa-solid fa-building text-indigo-400"></i> {{ $lowongan->perusahaan->name ?? 'Perusahaan Seeder' }}
                    </p>
                </div>
                        ...
                        @if(is_array($lowongan->skill_dibutuhkan))
                            @foreach($lowongan->skill_dibutuhkan as $skill)
                                <span class="inline-flex items-center rounded-lg bg-indigo-50 border border-indigo-100/60 px-3 py-1.5 text-xs font-bold text-indigo-700 shadow-sm">
                                    <i class="fa-solid fa-code text-[10px] mr-1.5 opacity-60"></i> {{ $skill }}
                                </span>
                            @endforeach
                        @else
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-3xl bg-white border border-slate-200/80 shadow-sm overflow-hidden p-6 space-y-5">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex justify-between items-center text-xs">
                        <span class="font-semibold text-slate-400">Kecamatan</span>
                        <span class="font-bold text-slate-800">{{ $lowongan->kecamatan ?? 'N/A' }}</span>
                    </div>
                    <div class="flex justify-between items-center text-xs">
                ...
                @if(auth()->user()->isJobSeeker())
                    @if($lowongan->status == 'aktif')
                        <a href="mailto:{{ $lowongan->perusahaan->email ?? 'recruitment@ematchkerja.test' }}?subject=Melamar Posisi: {{ $lowongan->posisi }} - e-MatchKerja" 
                           class="w-full inline-flex items-center justify-center rounded-xl bg-indigo-600 px-4 py-3 text-xs font-bold text-white shadow-md shadow-indigo-100 hover:bg-indigo-700 transition">
                            <i class="fa-regular fa-paper-plane mr-2"></i> Lamar Pekerjaan Sekarang
                        </a>
                    @else
                ...
                @if(auth()->user()->isEmployer() && $lowongan->perusahaan_id === auth()->id())
            ...
            <div class="rounded-3xl bg-gradient-to-tr from-indigo-900 to-slate-900 p-6 text-white space-y-3">
                ...
                <p class="text-[11px] text-slate-300 leading-relaxed">
                    Pastikan Anda telah mengisi **Profil Saya** terlebih dahulu sebelum melamar agar CV dan NIK Anda dapat ditinjau oleh perusahaan secara terpadu melalui platform e-MatchKerja.
                </p>
            </div>
        </div>
